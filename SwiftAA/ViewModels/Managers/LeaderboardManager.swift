...
class LeaderboardManager: ObservableObject {
    ...
    private func getEntriesForOtherVersions(version: Version) {
        if let versions = otherVersionData.first?.components(separatedBy: ","),
           let index = versions.firstIndex(of: "\(version.label) RSG") {
            let newEntries = otherVersionData.dropFirst(2)
                .compactMap({ createEntry(from: $0, index: index) }).filter({ !$0.name.isEmpty })
            updateEntries(newEntries: newEntries)
        }
    }
